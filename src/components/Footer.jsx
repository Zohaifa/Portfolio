...
const Footer = () => {
  ...
  return (
    <footer className="bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white py-16 px-6 border-t border-sky-500/30">
          ...
          {footer.contactCards.map((card) => {
            ...
            return (
              <div key={card.type} className="bg-gradient-to-br from-gray-800 to-gray-900 p-6 rounded-lg border border-sky-500/30 hover:border-sky-500 transition-all duration-300 shadow-lg hover:shadow-sky-500/20">
                ...
                {details.href ? (
                  <a href={details.href} className="text-sky-400 hover:text-sky-300 transition-colors">
                    {details.value}
                  </a>
                ) : (
                  <p className="text-sky-400">{details.value}</p>
                )}
              </div>
            );
          })}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-gray-800">
          ...
          <div className="flex gap-6">
            {socialLinks.map((social) => (
              <a key={social.type} href={social.url} className="text-gray-400 hover:text-sky-400 transition-colors">
